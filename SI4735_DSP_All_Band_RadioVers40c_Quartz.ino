//  V4.0c  - 29-07-2022 Binns Version 4.0 Modified by YO2LDK - Alex for ILI9488 4" TFT LCD
//         - (retro version full screen); new S-meter interface;
//         - I added the NTP dual clock function (UTC and Local), 
//           is very useful for HAM's , also NTP syncronization display,green online,red offline, yellow wainting for
//         - with FULL RDS  text support added by work of my friend IU4ALH - Antonino ;
//
...
void setup() {
  ...
  tft.setRotation(1);
  uint16_t calData[5] = { 205, 3632, 234, 3418, 7 };  
...
if (( currentMode == FM ) or ( currentMode == AM ) or ( currentMode == USB )or ( currentMode == LSB )and ((FirstLayer) or (ThirdLayer) or (SecondLayer))) { 
 ...
 my_time.setLocation("Europe/Belgrade");   // !!! See https://en.wikipedia.org/wiki/List_of_tz_database_time_zones
 utc_time = now();
 loc_time = my_time.now(); 
...
void NTS()
{
  ...
  #define YELLOW      RGB(254, 254,   0)
  #define BLUE        RGB(  0,   0, 128)

  if ( second()%30 ) return;                         
  sync_age = now() - lastNtpUpdateTime();            
  if ( sync_age < SYNC_MARGINAL )                   
    sync_result = GREEN;
  else 
  if ( sync_age < SYNC_LOST )                        
    sync_result = YELLOW;
  else 
    sync_result = RED;                              
...
void showRDSStation() {
  ...
  if ((FirstLayer) or (ThirdLayer)) {
    tft.setCursor(50, YFreqDispl + 80);
    tft.setTextSize(3);
    tft.setTextColor(TFT_GOLD, TFT_NAVY);
    tft.print("Radio  ");
    tft.setCursor(XFreqDispl + 220, YFreqDispl + 80);
    tft.setTextSize(3);
    tft.setTextColor(TFT_YELLOW, TFT_NAVY);
...
void drawRETRO() {
  ...
  tftRusPrint(String(presetNameLoad() + " City"), 83, 32);
  tftRusSetCut(0, 0);
   if (tftRusLength(String(presetNameLoad() + " City")) > (10 + (10))) {  
    textScroll = 1;
    elapsedScroll = millis() + 2000;
  }
  //band name
